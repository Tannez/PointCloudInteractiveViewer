...
namespace BAPointCloudRenderer.Loading {
    ...
    class CloudLoader
    {
        ...
        private static void LoadPoints(string dataRPath, PointCloudMetaData metaData, Node node)
        {
            ...
            int offset = 0; // related to byte width

            Vector3[] vertices = new Vector3[numPoints];
            Color[] colors = new Color[numPoints];
            float[] intensities = new float[numPoints];
            int[] classification = new int[numPoints];
            ...
            foreach (PointAttribute pointAttribute in metaData.pointAttributesList)
            {
                ...
                else if (pointAttribute.name.Equals(PointAttributes.CLASSIFICATION))
                {
                    for (int i = 0; i < numPoints; i++)
                    {
                        classification[i] = data[offset + i * pointByteSize];
                    }
                    offset += 1;
                }
                // Intensity Attribute Condition
                else if (pointAttribute.name.Equals(PointAttributes.INTENSITY))
                {
                    for (int i = 0; i < numPoints; i++)
                    {
                        intensities[i] = data[offset + i * pointByteSize];
                    }
                    offset += 2;
                }

            }
            node.SetPoints(vertices, colors, classification, intensities);
        }
        ...
        // Script changes suggested by Akharroubi for changing color of points
        // https://github.com/SFraissTU/BA_PointCloud/issues/9
        public static int ByteSize(PointCloudMetaData metaData)
        {
            string[] Attributes = new string[] { "POSITION_CARTESIAN", "COLOR_PACKED", "intensity",
                                                "classification", "SOURCE_ID" };
            int[] Size = new int[] { 12, 4, 2, 1, 2 };
            int pointByteSize = 0;
            foreach (PointAttribute pointAttribute in metaData.pointAttributesList)
            {
                for (int i = 0; i < Attributes.Length; i++)
                {
                    if (pointAttribute.Equals(Attributes[i]))
                    {
                        pointByteSize += Size[i];
                    }
                }
            }
