...
namespace BAPointCloudRenderer.ObjectCreation {
    ...
    /// <summary>
    /// Color Scheme to display point cloud <- added enum
    enum ColorMode
    {
        RGBA,
        Intensity,
    ...
    class DefaultMeshConfiguration : MeshConfiguration
    {
        ...
        private void LoadShaders()
        {
            ...
            if (interpolation == FragInterpolationMode.OFF)
            {
                if (screenSize)
                {
                    ...
                    // Debug.Log("Using Shader QuadGeoScreenSizeShader");
                }
                else
                {
                    material = new Material(Shader.Find("Custom/QuadGeoWorldSizeShader"));
                    // Debug.Log("Using Shader QuadGeoWorldSizeShader");
                }
            }
            else if (interpolation == FragInterpolationMode.PARABOLOIDS || interpolation == FragInterpolationMode.CONES)
            {
                if (screenSize)
                {
                    ...
                    // Debug.Log("Using Shader ParaboloidFragScreenSizeShader");
                }
                else
                {
                    material = new Material(Shader.Find("Custom/ParaboloidFragWorldSizeShader"));
                    // Debug.Log("Using Shader ParaboloidFragWorldSizeShader");
                }
        ...
        public override GameObject CreateGameObject(string name, Vector3[] vertexData, Color[] colorData, int[] classData, float[] intensityData, BoundingBox boundingBox, Transform parent)
        {
        ...
        // hardcoded classes for various classification categories. If more needed, add more cases.
        private Color ClassificationColor(int cls)
        {
            switch (cls)
            {
