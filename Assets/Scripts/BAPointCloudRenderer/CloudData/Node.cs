...
namespace BAPointCloudRenderer.CloudData {
    ...
    public class Node : IEnumerable<Node> {
        ...
        //The classifications this node is containing. Null before being set. <- Added to code
        private int[] classificationToStore;
        //The Intensities this node is containing. Null before being set.<- Added to code
        private float[] intensitiesToStore;
        ...
        public void CreateGameObjects(MeshConfiguration configuration, Transform parent) {
            ...
            if (verticesToStore.Length <= max) {
                gameObjects.Add(configuration.CreateGameObject(metaData.cloudName + "/" + "r" + name + " (" + verticesToStore.Length + ")", verticesToStore, colorsToStore, classificationToStore, intensitiesToStore, boundingBox, parent));
            } else {
                ...
                int[] restClass = classificationToStore; // <- Added
                float[] restIntensity = intensitiesToStore; // <- Added
                while (amount > 0)
                {
                    Vector3[] vertices = restVertices.Take(amount).ToArray();
                    Color[] colors = restColors.Take(amount).ToArray();
                    int[] classifications = restClass.Take(amount).ToArray(); // <- Added
                    float[] intensities = restIntensity.Take(amount).ToArray(); // <- Added
                    restVertices = restVertices.Skip(amount).ToArray();
                    restColors = restColors.Skip(amount).ToArray();
                    restClass = restClass.Skip(amount).ToArray(); // <- Added
                    restIntensity = restIntensity.Skip(amount).ToArray(); // <- Added
                    gameObjects.Add(configuration.CreateGameObject(metaData.cloudName + "/" + "r" + name + "_" + index + " (" + vertices.Length + ")", vertices, colors, classifications, intensities, boundingBox, parent));
        ...
        public void SetPoints(Vector3[] vertices, Color[] colors, int[] classification, float[] intensities) {
            if (gameObjects.Count != 0) {
                throw new ArgumentException("GameObjects already created!");
            }
            if (vertices == null || colors == null || classification == null || intensities == null || vertices.Length != colors.Length) {
                throw new ArgumentException("Invalid data given!");
            }
            verticesToStore = vertices;
            colorsToStore = colors;
            classificationToStore = classification; // Added parameter
            intensitiesToStore = intensities; // Added parameter
        ...
        public void ForgetPoints()
        {
            ...
            colorsToStore = null;
            classificationToStore = null;
            intensitiesToStore = null;
        }
        ...
        public bool HasPointsToRender() {
            return verticesToStore != null && colorsToStore != null && classificationToStore != null && intensitiesToStore != null && pointCount != -1;
        }
        ...
        // Added List
        public float[] IntensitiesToStore
        {
            get { return intensitiesToStore;}
        }
        // Added List
        public int[] ClassificationToStore
        {
            get { return classificationToStore; }
        }
